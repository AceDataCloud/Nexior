  ...
  <el-row class="header">
    ...
    <el-col :md="4" :xs="11">
      <div v-if="!authenticated" class="mt-4 pr-10">
        ...
        <locale-selector class="locale float-right mr-4" />
        <dark-selector class="float-right mr-4" />
      </div>
      <div v-else class="float-right">
        <dark-selector class="mr-4" />
        <locale-selector class="locale" />
...
export default defineComponent({
  ...
  methods: {
    ...
    onLogin() {
      if (this.$config.auth.flow === 'popup') {
        this.$store.dispatch('setShowLogin', true);
      } else if (this.$config.auth.flow === 'redirect') {
        this.$router.push({
