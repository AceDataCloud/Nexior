<template>
  <div class="preview">
    <div class="left">
      <capability-presentation capability="qwenimage" part="avatar" class="avatar" />
    </div>
    <div class="main">
      <div class="bot">
        <capability-presentation capability="qwenimage" part="name" />
        <span class="datetime">
      ...
      <div class="info">
        <div
          v-if="Array.isArray(modelValue?.request?.image_urls) && modelValue?.request?.image_urls.length > 0"
        ...
        <p v-if="modelValue?.request?.prompt" class="prompt mt-2">
          ...
          <span v-if="!modelValue?.response"> - ({{ $t('qwenimage.status.pending') }}) </span>
        </p>
      </div>
      ...
      <div v-else-if="modelValue?.response?.success === true" :class="{ content: true, failed: true }">
        ...
        <div :class="{ operations: true, 'mt-2': true, 'mb-2': true }">
          ...
          <api-code-button path="/qwenimage/images" :body="apiRequest" />
          <report-button
            service="qwenimage"
        ...
        <el-alert :closable="false" class="mt-2 success">
          ...
          <p v-if="modelValue?.elapsed" class="text-[var(--el-text-color-regular)] text-xs mb-2">
            <time-icon class="mr-1" :size="'1em' as any" aria-hidden="true" focusable="false" />
            {{ $t('qwenimage.name.elapsed') }}: {{ modelValue?.elapsed?.toFixed(2) }}s
          </p>
      ...
      <div v-else-if="modelValue?.response?.success === false" class="content">
        <el-alert :closable="false" class="failure">
          ...
          <p v-if="modelValue?.response?.error?.message" class="text-[var(--el-text-color-regular)] text-xs mb-2">
            ...
            {{ $t('qwenimage.name.failureReason') }}:
...
import { IQwenImageTask, IQwenImageImage } from '@/models';
...
import { getQwenImageShortModel } from '@/constants';
import { buildQwenImageRequest } from '@/utils/qwenimage/request';

export default defineComponent({
  name: 'QwenImageTaskPreview',
  components: {
    DeleteIcon,
    ApplicationIcon,
    ChannelIcon,
  ...
  props: {
    modelValue: {
      type: Object as () => IQwenImageTask | undefined,
      required: true
    }
  },
  computed: {
    apiRequest() {
      return buildQwenImageRequest(this.modelValue?.request);
    },
    isEdit(): boolean {
      return (
        Array.isArray(this.modelValue?.request?.image_urls) && (this.modelValue?.request?.image_urls?.length || 0) > 0
      );
    },
    images(): IQwenImageImage[] {
      const result: IQwenImageImage[] = [];
      // @ts-ignore
      if (Array.isArray(this.modelValue?.response?.data)) {
        this.modelValue?.response?.data?.forEach((item: any) => {
          result.push(item as IQwenImageImage);
        });
      }
      return result;
    }
  ...
  methods: {
    async onDelete() {
      ...
      try {
        await this.$store.dispatch('qwenimage/deleteTask', { id });
    ...
    shortModel(model?: string) {
      return getQwenImageShortModel(model) || model;
    },
    onEdit(imageUrl?: string) {
      if (!imageUrl) return;
      console.debug('Edit image:', imageUrl);
      const nextConfig = { ...(this.$store.state.qwenimage?.config || {}) };
      (nextConfig as any).image = [imageUrl];
      this.$store.commit('qwenimage/setConfig', nextConfig);
    }
  }
});
</script>
