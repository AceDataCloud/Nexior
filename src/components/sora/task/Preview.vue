    ...
    <div class="main">
      ...
      >
        ...
        <div v-if="modelValue?.response.success" :class="{ operations: true, 'mt-2': true }">
          <el-tooltip class="box-item" effect="dark" :content="$t('sora.message.downloadVideo')" placement="top-start">
            <el-button
            ...
            >
              {{ $t('sora.button.download') }}
            </el-button>
          </el-tooltip>
          <api-code-button path="/sora/videos" :body="modelValue?.request" />
        </div>
...
export default defineComponent({
  ...
  props: {
    modelValue: {
      type: Object as () => ISoraTask | undefined,
      required: true
    }
  },
  data() {
  ...
  computed: {
    application() {
      return this.$store.state.sora?.application;
    },
    config() {
