  ...
  <div class="relative">
    <div class="flex justify-between">
      <div class="flex justify-start items-center">
        <span class="text-sm font-bold">{{ $t('sora.name.startEndImage') }}</span>
        <info-icon :content="$t('sora.description.uploadStartEndImage')" />
      </div>
    </div>
    <el-upload
...
export default defineComponent({
  ...
  computed: {
    ...
    value: {
      get() {
        return this.$store.state?.sora?.config?.image_urls;
      },
      set() {
  ...
  methods: {
    onExceed() {
      ElMessage.warning(this.$t('sora.message.uploadReferencesExceed'));
    },
    onError() {
      ElMessage.error(this.$t('sora.message.uploadReferencesError'));
    },
