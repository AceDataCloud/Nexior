<template>
  <div>
    <span class="text-sm font-bold mb-2 block">{{ $t('pixverse.name.aspectRatio') }}</span>
    <div class="items">
      <div
...
import { PIXVERSE_DEFAULT_ASPECT_RATIO } from '@/constants';
...
export default defineComponent({
  ...
  computed: {
    ...
    value: {
      get() {
        return this.$store.state.pixverse.config?.aspect_ratio;
      },
      set(val: string) {
  ...
  mounted() {
    if (!this.value) {
      this.value = PIXVERSE_DEFAULT_ASPECT_RATIO;
    }
  }
});
</script>
