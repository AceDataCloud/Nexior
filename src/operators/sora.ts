...
class SoraOperator {
  async task(id: string, options: { token: string }): Promise<AxiosResponse<ISoraTaskResponse>> {
  ...
  ): Promise<AxiosResponse<ISoraTasksResponse>> {
    return await axios.post(
      `/sora/tasks`,
      {
  ...
  async generate(
    data: ISoraGenerateRequest,
    options: {
      token: string;
    }
  ): Promise<AxiosResponse<ISoraGenerateResponse>> {
    return await axios.post('/sora/videos', data, {
      headers: {
        authorization: `Bearer ${options.token}`,
        'content-type': 'application/json',
