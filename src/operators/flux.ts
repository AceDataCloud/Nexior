...
class FluxOperator {
  async task(id: string, options: { token: string }): Promise<AxiosResponse<IFluxTaskResponse>> {
  ...
  ): Promise<AxiosResponse<IFluxTasksResponse>> {
    return await axios.post(
      `/flux/tasks`,
      {
  ...
  ): Promise<AxiosResponse<IFluxGenerateResponse>> {
