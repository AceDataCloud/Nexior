...
class SeedreamOperator {
  async task(id: string, options: { token: string }): Promise<AxiosResponse<ISeedreamTaskResponse>> {
  ...
  ): Promise<AxiosResponse<ISeedreamTasksResponse>> {
    return await axios.post(
      `/seedream/tasks`,
      {
  ...
  async generate(
    data: ISeedreamGenerateRequest,
    options: {
      token: string;
    }
  ): Promise<AxiosResponse<ISeedreamGenerateResponse>> {
    return await axios.post('/seedream/images', data, {
      headers: {
        authorization: `Bearer ${options.token}`,
        'content-type': 'application/json',
