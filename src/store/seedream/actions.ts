import { applicationOperator, credentialOperator, seedreamOperator, serviceOperator } from '@/operators';
...
export const getService = async ({
  commit,
  state
}: ActionContext<ISeedreamState, IRootState>): Promise<IService | undefined> => {
  state.status.getService = Status.Request;
  try {
    const { data: service } = await serviceOperator.get(SEEDREAM_SERVICE_ID);
...
export const setConfig = ({ commit }: any, payload: ISeedreamConfig) => {
...
export const setTasksItems = ({ commit }: any, payload: ISeedreamTask[]) => {
...
export const setTasksActive = ({ commit }: any, payload: ISeedreamTask) => {
