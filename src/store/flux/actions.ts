import { applicationOperator, credentialOperator, fluxOperator, serviceOperator } from '@/operators';
...
export const getService = async ({
  commit,
  state
}: ActionContext<IFluxState, IRootState>): Promise<IService | undefined> => {
  state.status.getService = Status.Request;
  try {
    const { data: service } = await serviceOperator.get(FLUX_SERVICE_ID);
