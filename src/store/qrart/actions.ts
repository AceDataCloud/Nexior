import { applicationOperator, credentialOperator, qrartOperator, serviceOperator } from '@/operators';
...
export const getService = async ({
  commit,
  state
}: ActionContext<IQrartState, IRootState>): Promise<IService | undefined> => {
  state.status.getService = Status.Request;
  try {
    const { data: service } = await serviceOperator.get(QRART_SERVICE_ID);
