import { applicationOperator, credentialOperator, soraOperator, serviceOperator } from '@/operators';
...
export const resetAll = ({ commit }: ActionContext<ISoraState, IRootState>): void => {
  commit('resetAll');
};
...
export const getService = async ({
  commit,
  state
}: ActionContext<ISoraState, IRootState>): Promise<IService | undefined> => {
  state.status.getService = Status.Request;
  try {
    const { data: service } = await serviceOperator.get(SORA_SERVICE_ID);
...
export const setConfig = ({ commit }: any, payload: ISoraConfig) => {
...
export const setTasksItems = ({ commit }: any, payload: ISoraTask[]) => {
...
export const setTasksActive = ({ commit }: any, payload: ISoraTask) => {
