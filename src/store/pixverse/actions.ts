import { applicationOperator, credentialOperator, pixverseOperator, serviceOperator } from '@/operators';
...
export const resetAll = ({ commit }: ActionContext<IPixverseState, IRootState>): void => {
  commit('resetAll');
};
...
export const getService = async ({
  commit,
  state
}: ActionContext<IPixverseState, IRootState>): Promise<IService | undefined> => {
  state.status.getService = Status.Request;
  try {
    const { data: service } = await serviceOperator.get(PIXVERSE_SERVICE_ID);
...
export const getApplications = async ({
  ...
  try {
    const { data: applications } = await applicationOperator.getAll({
      ...
      service_id: PIXVERSE_SERVICE_ID
    });
...
export const setConfig = ({ commit }: any, payload: IPixverseConfig) => {
...
export const setTasksItems = ({ commit }: any, payload: IPixverseTask[]) => {
...
export const setTasksActive = ({ commit }: any, payload: IPixverseTask) => {
