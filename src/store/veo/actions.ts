import { applicationOperator, credentialOperator, veoOperator, serviceOperator } from '@/operators';
...
export const resetAll = ({ commit }: ActionContext<IVeoState, IRootState>): void => {
  commit('resetAll');
};
...
export const getService = async ({
  commit,
  state
}: ActionContext<IVeoState, IRootState>): Promise<IService | undefined> => {
  state.status.getService = Status.Request;
  try {
    const { data: service } = await serviceOperator.get(VEO_SERVICE_ID);
...
export const setConfig = ({ commit }: any, payload: IVeoConfig) => {
...
export const setTasksItems = ({ commit }: any, payload: IVeoTask[]) => {
...
export const setTasksActive = ({ commit }: any, payload: IVeoTask) => {
