import { applicationOperator, credentialOperator, hailuoOperator, serviceOperator } from '@/operators';
...
export const getService = async ({
  commit,
  state
}: ActionContext<IHailuoState, IRootState>): Promise<IService | undefined> => {
  state.status.getService = Status.Request;
  try {
    const { data: service } = await serviceOperator.get(HAILUO_SERVICE_ID);
