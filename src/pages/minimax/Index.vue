...
export default defineComponent({
  ...
  computed: {
    applicationsLoading() {
      return this.$store.state.minimax?.status?.getApplications === Status.Request;
    },
    tasksLoading() {
      return this.$store.state.minimax?.status?.getTasks === Status.Request || this.fetchingTasks;
    },
    credential() {
      return this.$store.state.minimax.credential;
    },
    config() {
  ...
  methods: {
    ...
    async onGetService() {
      ...
      await this.$store.dispatch('minimax/getService');
      console.debug('end onGetService');
    },
    async onGetApplication() {
      console.debug('start onGetApplication');
      await this.$store.dispatch('minimax/getApplications');
    ...
    async onGenerate() {
      ...
      instrumentGeneration('minimax', operation)
        ...
        .catch((error) => {
          ...
          if (showQuotaExhausted(error, 'minimax')) return;
          if (this.walletMode) {
